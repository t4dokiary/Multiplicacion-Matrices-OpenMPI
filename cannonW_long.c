...
main(int argc, char* argv[]) {
    ...
    //rint_matrix("Se leyó A =", local_A, &grid, n); //muestra la matriz resultante
    ...
    if (grid->my_rank == 0) {
        ...
        for (mat_row = 0;  mat_row < n; mat_row++) {
            ...
            for (grid_col = 0; grid_col < grid->q; grid_col++) {
                ...
                if (source == 0) {
                    for(mat_col = 0; mat_col < Order(local_A); mat_col++)
                        printf("%25ld", Entry(local_A, mat_row, mat_col));
                } else {
                    MPI_Recv(temp, Order(local_A), MPI_FLOAT, source, 0, grid->comm, &status);
                    for(mat_col = 0; mat_col < Order(local_A); mat_col++)
                        printf("%25ld", temp[mat_col]);
                }
            }
            printf("\n");
        }
